...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///dernek_otomasyon.db'
...
class Bagis(db.Model):
    bagis_id = db.Column(db.Integer, unique=True, primary_key=True, autoincrement=True)
    bagis_yapan = db.Column(db.String(500), nullable=False)
    bagis_yonetici_id = db.Column(db.Integer, db.ForeignKey('uye.uye_id'), nullable=False)
    bagisyonetici = db.relationship('Uye', backref=db.backref('bagisyonetici', lazy=True),
                                    foreign_keys=[bagis_yonetici_id])

    bagis_tutari = db.Column(db.Float, nullable=False)
    bagis_tarihi = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    bagis_aciklama = db.Column(db.String(500), nullable=False)
